...
<head>
  ...
  <style>
    :root{ --bg:#f6f8fb; --card:#ffffff; --muted:#6b7280; --accent:#2563eb; --accent-2:#0b61d8; --danger:#ef4444; --success:#10b981; --shadow: 0 8px 28px rgba(20,20,50,0.06); font-family:Inter, system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial; }
    ...
    .modal{background:var(--card);width:720px;border-radius:12px;padding:18px;box-shadow:var(--shadow)}
...
<body>
    ...
    <main class="main">
      <div class="topbar">
        <div>
          ...
          <div class="mini muted">Create, edit and manage inventory items</div>
        </div>
      ...
      <section class="card">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px">
          ...
          <div class="right mini muted">Page <span id="pageDisplay">1</span></div>
        </div>
  ...
  <script>
    const api = axios.create({ baseURL: '<?= site_url('api/items'); ?>', headers: { 'X-Requested-With': 'XMLHttpRequest' } });
    ...
    async function loadItems(){
      ...
      try{
        ...
        state.total = pag.total; state.total_pages = pag.total_pages; state.per_page = pag.per_page; state.page = pag.page; state.offset = pag.offset || ((state.page-1)*state.per_page);

        const tbody = document.getElementById('itemsBody');
        if(!items.length){ tbody.innerHTML = '<tr><td colspan="8" class="mini muted">No items found.</td></tr>'; renderPager(); document.getElementById('countDisplay').innerText = state.total; return; }

        const startSerial = state.offset + 1;
        tbody.innerHTML = items.map((it, idx) => (`\n          <tr id="row-${it.id}">\n            <td>${startSerial + idx}</td>\n            <td>${it.id}</td>\n            <td><strong>${esc(it.name)}</strong></td>\n            <td class="mini">${esc(it.sku||'')}</td>\n            <td>${it.quantity}</td>\n            <td>${Number(it.price).toFixed(2)}</td>\n            <td class="mini">${it.category_id ?? ''}</td>\n            <td>\n              <button class="ghost" onclick="openEdit(${it.id})">Edit</button>\n              <button class="warn" onclick="onDelete(${it.id})">Delete</button>\n            </td>\n          </tr>\n        `)).join('');
        ...
        renderPager();
      }catch(err){ setMsg(listMsg, err?.response?.data?.error || err?.message || 'Failed to load', false); }
    }
    ...
    function goToPage(p){ if(p<1) p=1; if(state.total_pages && p>state.total_pages) p=state.total_pages; state.page=p; state.q = document.getElementById('qInput').value.trim(); state.per_page = parseInt(document.getElementById('perPageSelect').value,10)||10; loadItems(); }
    ...
    function openCreateModal(){ document.getElementById('modalTitle').innerText='Add Item'; document.getElementById('itemForm').reset(); document.getElementById('item_id').value=''; setMsg(document.getElementById('createMsg'),''); showModal(true); }
    function openEditModalData(item){ document.getElementById('modalTitle').innerText='Edit Item'; document.getElementById('item_id').value = item.id; document.getElementById('c_name').value = item.name; document.getElementById('c_sku').value = item.sku || ''; document.getElementById('c_quantity').value = item.quantity; document.getElementById('c_price').value = item.price; document.getElementById('c_description').value = item.description || ''; document.getElementById('c_category_id').value = item.category_id || ''; setMsg(document.getElementById('createMsg'),''); showModal(true); }
    function closeModal(){ showModal(false); }

    /* Create/Update */
    async function onCreateOrUpdate(e){ e.preventDefault(); const id = document.getElementById('item_id').value; const payload = { name: document.getElementById('c_name').value.trim(), sku: document.getElementById('c_sku').value.trim(), description: document.getElementById('c_description').value.trim(), quantity: Number(document.getElementById('c_quantity').value), price: Number(document.getElementById('c_price').value), category_id: document.getElementById('c_category_id').value ? Number(document.getElementById('c_category_id').value) : '' };
      try{
    ...
    async function openEdit(id){ try{ const { data } = await api.get('/'+id); openEditModalData(data); }catch(err){ alert(err?.response?.data?.error || err?.message || 'Failed to fetch'); } }

    async function onDelete(id){ if(!confirm('Delete item '+id+'?')) return; try{ await api.post('/'+id, {_method:'DELETE'}); setMsg(document.getElementById('listMsg'),'Deleted', true); loadItems(); }catch(err){ setMsg(document.getElementById('listMsg'), err?.response?.data?.error || err?.message || 'Delete failed', false); } }
